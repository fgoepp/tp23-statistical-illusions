lindleyUI <- function(id) {
  ...
  sideBarInput <- tagList(
    ...
    # select actual data
    sliderInput(ns("n"), "n (= |samples|):", min = 2, max = 1000, value = 999),
    sliderInput(ns("p"), "p (= relative frequency):", min = 1, max = 99, value = 50),
    ...
    sliderInput(ns("H0_p"), "H₀ ratio:", min = 1, max = 99, value = 54),
